...
int main(int argc, char* argv[]) {
    ...
    if (!testbench_mode) {
        ...
        std::string log_name = "logs_device/";
    ...
    transpose_simple_kernel<<<DimGrid, DimBlock>>>(
        d_input, d_output, dim[0], dim[1], dim[2], perm[0], perm[1], perm[2]);
    ...
    process("Transpose Naive", testbench_mode, h_gold, h_output, size,
    ...
    process("Transpose with shared-memory (bank conflict free)", testbench_mode,
            h_gold, h_output, size, device_ms, host_ms, log);
